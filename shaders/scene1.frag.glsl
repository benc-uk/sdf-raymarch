...
Hit mapThing2(vec3 p) {
  ...
  cubeD = opSub(sdSphere(hole1Pos, holeRadius), cubeD);
  cubeD = opSub(sdSphere(hole2Pos, holeRadius), cubeD);
  cubeD = opSub(sdSphere(hole3Pos, holeRadius), cubeD);
  cubeD = opSub(sdSphere(hole4Pos, holeRadius), cubeD);
...
void main() {
  ...
  materials[1] = Material(vec3(0.2, 0.7, 0.1), 0.8, 0.3, 6.0, false); // Greenish
  ...
  // First check for ground plane intersection analytically
  float groundT = -1.0;
  if(rd.y < 0.0) {
    groundT = -ro.y / rd.y;
  }
  ...
  // Check if ground plane is closer than scene objects
  if(groundT > 0.0 && (hit.matID == -1 || groundT < t)) {
    t = groundT;
    p = ro + rd * t;
    hit = Hit(0.0, 0); // Ground material ID
  }
  ...
  if(hit.matID != -1) {
    ...
    col = materials[hit.matID].color * diff;

    // specular highlight
    vec3 viewDir = normalize(ro - p);
    vec3 reflectDir = reflect(-lightDir, n);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), materials[hit.matID].hardness);
    col += vec3(0.7) * spec * materials[hit.matID].specular;
